...
function search_branches(search_method::BFS, split_method, prop_method, problem)
    ...
    for iter in 1:search_method.max_iter # BFS with max iteration
        ...
        if length(batch_input) >= search_method.batch_size || length(branches) == 0
            # batch_bound, batch_info = propagate(prop_method, problem.Flux_model, batch_input, batch_output, batch_info)
            # batch_result = check_inclusion(prop_method, problem.Flux_model, batch_input, batch_bound, batch_output)
            batch_bound, batch_out_spec, batch_info = prepare_method(prop_method, problem.Flux_model, batch_input, batch_output, batch_info)
            batch_bound, batch_info = propagate(prop_method, problem.onnx_model_path, problem.Flux_model, problem.input_shape, batch_bound, batch_out_spec, batch_info)
            ...
            for i in eachindex(batch_input)
                ...
                sub_branches = split_branch(split_method, problem.Flux_model, batch_input[i], batch_output[i], batch_info[i])
