...
input = Hyperrectangle(low = [-0.9], high = [0.9])
output = Hyperrectangle(low = [30.0], high = [80.0])
info = nothing
branch = branching_method(:nothing, 100)
method_ai2 = method(Ai2(), :forward, branch)
ans = propagate(method_ai2, flux_model, input, output, info)
#forward_reach, info = forward(flux_model, input, Ai2(), nothing)
print(ans)