...
function propagate_layer_batch(prop_method::IBP, layer::Dense, bound::IBPBound, batch_info)
    bound_center = (bound.batch_low + bound.batch_up) ./ 2
    bound_radius = bound.batch_up - bound.batch_low
    @assert all(bound.batch_up - bound.batch_low .>= 0)
    c = layer.weight * bound_center + layer.bias
    r = abs.(layer.weight) * bound_radius
    return IBPBound(c-r, c+r)
end  

function propagate_layer(prop_method::RotatedBox, layer::Dense, reach::LazySet, batch_info)
    v_list = vertices_list(reach)
    v_list = [layer.weight * v for v in v_list]
    rb = rotated_hyperrect_overapprox(v_list)
    nv_list = vertices_list(rb)
    # @show nv_list
    # @show [rb.M' * v for v in nv_list]
    return rb
end

function rotated_hyperrect_overapprox(points::Vector{<:AbstractVector})
    # Convert the list of vectors to a matrix where each column is a point
    points_matrix = hcat(points...)
    
    # Step 1: Translate points to the origin by subtracting the centroid
    centroid = mean(points_matrix, dims=2)
    translated_points = points_matrix .- centroid
    
    # Step 2: Compute the covariance matrix of the translated points
    covariance_matrix = cov(translated_points')
    
    # Step 3: Perform eigenvalue decomposition on the covariance matrix
    eigenvalues, eigenvectors = eigen(covariance_matrix)
    M = eigenvectors  # Orthogonal rotation matrix (eigenvectors of the covariance matrix)
    # Step 4: Rotate the points using M (project points onto the principal components)
    rotated_points = M' * points_matrix
    
    # Step 5: Compute the dimension-wise bounds of the rotated points
    l = minimum(rotated_points, dims=2)
    u = maximum(rotated_points, dims=2)
