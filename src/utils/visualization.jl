...
function visualize_inference(problem::Problem, batch_size)
    model_info = onnx_parse(problem.onnx_model_path)
    xs = sample(problem.input, batch_size)
    batch_info = compute_output(model_info, xs)
    return model_info, batch_info
end
