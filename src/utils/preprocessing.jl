...
function prepare_problem(search_method::SearchMethod, split_method::SplitMethod, prop_method::PropMethod, problem::Problem)
    batch_info, global_info = onnx_parse(problem.onnx_model_path, problem.Flux_model, problem.input_shape)
    return batch_info, global_info, problem
end

function prepare_problem(search_method::SearchMethod, split_method::SplitMethod, prop_method::ImageStar, problem::Problem)
    batch_info, global_info = onnx_parse(problem.onnx_model_path, problem.Flux_model, problem.input_shape)
    return batch_info, global_info, Problem(problem.onnx_model_path, problem.Flux_model, problem.input_shape, init_bound(prop_method, problem.input), problem.output)
end
...
function onnx_parse(onnx_model_path, Flux_model, input_shape)
    ...
    global_info = Dict() # store the information of model, method and so on
