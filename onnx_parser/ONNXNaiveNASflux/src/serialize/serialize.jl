...
add_output!(ps::Vector{<:ProtoProbe}) = append!(ps[1].graph.output, map(p -> ONNX.ValueInfoProto(name(p), shape(p)), ps))
...
constant(x::Vector{<:AbstractProbe}, ::AbstractProbe, ns) = x
...
function (layer::Slice)(pp::AbstractProbe)
    fname = recursename("Slice", nextname(pp))
    # println(pp.name, pp.shape, nextname(pp), fname)
    # Add ONNX node info
    slice_range = collect(Tuple{Int, Int}, map(s -> s == Colon() ? (-1, -1) : (s[1], s[end]), layer.dims))
    slice_range = [element for tuple in slice_range for element in tuple]
    # println(slice_range)
    add!(pp, ONNXNaiveNASflux.BaseOnnx.NodeProto(
        # Names of input is provided by probes. This is why new probes need to be provided as output
        input=[name(pp)],
        # Name of output from this node
        output=[fname],
        name=fname,
        attribute = ONNXNaiveNASflux.BaseOnnx.AttributeProto.(["range"], [ONNXNaiveNASflux.BaseOnnx.TensorProto(slice_range, fname * "_slice_dims")]),
        op_type="Slice"))
    fluxshape = collect(Int, map(s -> s == Colon() ? -1 : length(s), layer.dims))
    fshape = function(s)
        return map(enumerate(fluxshape)) do (ind, new)
            new == -1 && return s[ind] # CBA to figure out how to do this...
            return new
        end |> Tuple
    end
