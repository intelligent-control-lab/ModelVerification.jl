...
verts[:Sub] = (name, inputs, params; kwargs...) -> elemwisevertex(name, inputs, params, -, 0; kwargs...)
...
verts[:Slice] = function(name, inputs, params; kwargs...)
    values = params[:range].int64_data
    dims = []
    for index in 1:2:length(values)
        if values[index] == -1
            push!(dims, Colon())
        else
            push!(dims, values[index]:values[index+1])
        end
    end
    return fluxvertex(name, Slice(Tuple(dims)), inputs...; kwargs...)
end
